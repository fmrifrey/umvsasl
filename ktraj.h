...
int genspiral(int N, int itr) {
	...
	if (N_stretch > MAXWAVELEN) {
		...
		/* Save values */
		grad_len = N;
		fprintf(stderr, "genspiral(): saving readout gradients with grad_len = %d\n", grad_len);
		fprintf(stderr, "genspiral(): saving readout x gradient with gx_max = %.2f G/cm\n", gx_max);
		fprintf(stderr, "genspiral(): saving readout y gradient with gy_max = %.2f G/cm\n", gy_max);
		fprintf(stderr, "genspiral(): saving readout z gradient with gz_max = %.2f G/cm\n", gz_max);
