classdef seq
    ...
    methods
        ...
        function obj = seq(varargin)
            ...
            obj.aqdims = [phdr.rdb.frame_size, phdr.rdb.user0, phdr.rdb.user2];
            ...
            obj.kspace = repmat(struct('data',[],'locs',[]), ...
                obj.aqdims(2), obj.aqdims(3));
            
            % Compress coils
            if args.ccfac < 1
                nvcoils = round(args.ccfac*obj.ncoils);
                raw = ir_mri_coil_compress(raw, 'ncoil', nvcoils);
                obj.ncoils = nvcoils;
            end
            
            % Loop through acquisition dimensions
            for i = 1:obj.aqdims(2)
                for j = 1:obj.aqdims(3)
    
                    % Get transformation matrix for current index
                    matidx = (i-1) * obj.aqdims(3) + j;
                    ...
                    obj.kspace(i,j) = struct( ...
                        'data', squeeze(raw(:,i,j,1,:)), ...
                        'locs', klocs ...
                        );
        ...
        function obj = setup_vol(obj,aqidx1,aqidx2)
            ...
            for i = aqidx1
                for j = aqidx2
                    klocs = [klocs; obj.kspace(i,j).locs];
                    kdat = [kdat; squeeze(obj.kspace(i,j).data)];
                end
            end
