...
long T_all[9][MAXNTRAINS*MAXNECHOES];
...
@inline Prescan.e PSfilter
	...
	acqs = 1;	/* Fixes the # of rhnpasses to the # of passes */
	acq_type = TYPGRAD;
@inline loadrheader.e rheaderinit   /* Recon variables */
	...
	scalerotmats( rsprot, &loggrd, &phygrd, ntrains*nechoes, obl_debug );
...
STATUS psdinit( void )
{
	...
	echon = trainn = 0;
	setrfconfig(ENBL_RHO1 + ENBL_THETA);	/* Only activate rho1 */
	setssitime( TIMESSI/GRAD_UPDATE_TIME );		/* Set ssi counter to 400 us. */
...
/**********************************************/
/* Runtime core section executed during scan. */
/**********************************************/
STATUS scanCore( void )
{

	int framen, echon, trainn, n;
	long T_0[9];
	long tmpmat[9];
	...
	setupslices( receive_freq1, rsp_info, opslquant,(float)0, echo1bw, opfov,
			TYPREC);	
	setfrequency(receive_freq1[opslquant/2] + receive_freq1[opslquant/2 - 1]/2, &echo1, 0);
	...
	if (ispre) {
		/* Set gradients to 0 amplitude */
		setiamp(0, &gxw, 0);
		setiamp(0, &gyw, 0);
		setiamp(0, &gzw, 0);

		/* Turn off the acquisition for disdaqs */
		...
		for (trainn = 0; trainn < rspdda; trainn++) {
			...
			for (echon = 0; echon < nechoes; echon++) {
				...
				/* Play spiral readout core (don't worry about rotations since gradients are off */
				boffset(off_seqcore);
				startseq( 0, (short)MAY_PAUSE );
				settrigger(TRIG_INTERN, 0);
			}
	...
	for (framen = 0; framen < (ispre) ? (1) : (nframes); framen++) {
		for (trainn = 0; trainn < (ispre) ? (rspvus) : (ntrains); trainn++) {
			...
			for (echon = 0; echon < nechoes; echon++) {
				...
				/* Load acquisition */
				loaddab(&echo1, echon, 0, 1,
					framen*ntrains + trainn,
				...
				/* Set rotation matrix for current echo */
				for (n = 0; n < 9; n++)
					tmpmat[n] = T_all[n][trainn*nechoes + echon];
				setrotate(tmpmat, 0);

				/* Play spiral readout core */
				boffset( off_seqcore );
				startseq( 0, (short)MAY_PAUSE );
				settrigger(TRIG_INTERN, 0);

				/* Restore the baseline transformation matrix */
				setrotate(T_0, 0);
			}
